...
- `cargo test --workspace` — **435 passed, 0 failed, 1 ignored.** Database-backed `utopia-store` integration tests skip in a database-less sandbox, matching the upstream `backend` CI job's documented behavior; the database-backed run is exercised by the upstream `migrations` CI job on every PR here.
- `docker compose --profile app up -d` — **not run: Docker is unavailable in this sandbox.** Per the bootstrap plan, the documented compose quick start is covered by the upstream CI that runs on every PR here: the `migrations` job applies the full migration sequence against `pgvector/pgvector:pg16`, and the `web` job installs and type-checks the UI with the pinned pnpm.
